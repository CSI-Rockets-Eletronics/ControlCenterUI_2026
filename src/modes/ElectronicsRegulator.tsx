...
export const ElectronicsRegulator = memo(function ElectronicsRegulator() {
  ...
  const pressureWarnings = useMemo(() => {
    ...
    const copv1Samples = telemetryStore.getSamples('copv_1_psi', recentTime, now);
    const copv2Samples = telemetryStore.getSamples('copv_2_psi', recentTime, now);
    const oxtank1Samples = telemetryStore.getSamples('oxtank_1_psi', recentTime, now);
    const oxtank2Samples = telemetryStore.getSamples('oxtank_2_psi', recentTime, now);
    const oxtank3Samples = telemetryStore.getSamples('oxtank_3_psi', recentTime, now);
  ...
  return (
    <div className="flex flex-col p-4 border bg-gray-bg-1 rounded-xl border-gray-border gap-3">
      ...
      {pressureWarnings.length > 0 && (
        <div className="px-2 py-1 border rounded-md bg-red-bg border-red-border flex items-center gap-2">
          <span className="text-red-text text-xs">🚩</span>
